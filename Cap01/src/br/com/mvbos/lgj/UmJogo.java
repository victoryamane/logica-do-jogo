...
public class UmJogo extends JFrame {
    ...
    private int pontos;
    private int larg = 50; // Largura padão
    ...
    private void atualizaJogo() {
        ...
        for (Elemento bloco : blocos) {
            ...
            if (colide(bloco, tiro) && bloco.y > 0) {
                bloco.y -= bloco.velociade * 2;
                tiro.y = bloco.y;
            } else {
                int sorte = r.nextInt(10);
                if (sorte == 0)
                    bloco.y += bloco.velociade + 1;
                else if (sorte == 5)
                    bloco.y -= bloco.velociade;
                else
                    bloco.y += bloco.velociade;
            }
        }
